...
export default function AdminFeedbackPage() {
  ...
  const { data, loading, error, refetch } = useAdminFetch<FeedbackData>(
  ...
  const handleDismiss = async (contentType: string, contentId: string): Promise<void> => {
    try {
      ...
      // Error will show on next refetch if the dismiss truly failed
    }
  };
  ...
  return (
    <AdminAuthGuard loading={status === 'loading'} session={session}>
      <div>
        ...
        {loading && <AdminSpinner label="Loading..." />}
